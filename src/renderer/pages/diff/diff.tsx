...
const ExcelDiff = () => {
  ...
  const fileInputChange = (
    setSheet: SetterFunction<SHEET_SETTER_TYPE[]>,
    setTitle: SetterFunction<string>
  ) => {
    return (event: any) => {
  ...
  return (
    <React.Fragment>
      {sheetIdx ? (
        <Radio.Group
          defaultValue={sheetIdx}
          buttonStyle="solid"
          onChange={indexChange}
        >
          {sheetIdxs.map((element) => (
            <Radio.Button value={element} key={element}>
              {element}
            </Radio.Button>
          ))}
        </Radio.Group>
      ...
      <Row>
        <Col span={12}>
          <input
            id={SHEET_TYPE.SRC.name}
            type="file"
            accept=".xls,.xlsx"
            style={{ display: 'none' }}
            onChange={fileInputChange(
              SHEET_TYPE.SRC.sheetSetter,
              SHEET_TYPE.SRC.titleSetter
            )}
          ></input>
          <Button
            icon={<UploadOutlined />}
            onClick={getFilds(SHEET_TYPE.SRC.name)}
          >
            Click to Upload
          </Button>
        </Col>
        <Col span={12}>
          <input
            id={SHEET_TYPE.DIFF.name}
            type="file"
            accept=".xls,.xlsx"
            style={{ display: 'none' }}
            onChange={fileInputChange(
              SHEET_TYPE.DIFF.sheetSetter,
              SHEET_TYPE.DIFF.titleSetter
            )}
          ></input>
          <Button
            icon={<UploadOutlined />}
            onClick={getFilds(SHEET_TYPE.DIFF.name)}
          >
            Click to Upload
          </Button>
        </Col>
